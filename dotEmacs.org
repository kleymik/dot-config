 ...
 (defun my-org-dft ()
   ...
   (cond ((eq (org-element-type oeap) 'headline)
         ...
         ((or (eq (org-element-type oeap) 'example-block) (looking-at "^#\\+RESULTS:")) ;; (eq (org-element-type oeap) 'fixed-width)
             (progn (message "Looking at RESULT")
...
|--------------+-----------+--------------+--------------+------+------------------+--------------+-----------+--------------------+------+-------------+-----------+-------+--------------|
| *eshell*     | doAndDone | ibuffers     | dired ~/     |      | app-open    file | window flip  | print var | babel exec block   |      | macro-start | tog node  | selv0 | edebug fun   |
|--------------+-----------+--------------+--------------+------+------------------+--------------+-----------+--------------------+------+-------------+-----------+-------+--------------|
| *bash*       | mySystems | dotEmacs     | dired repos/ | CTRL | open @point file | truncation   |           | babel exec & next  | CTRL | macro-go    | tog all   | selv+ | debug-off    |
| *python*     | scratch   | dotBashrc    | dired hist/  | SHFT | clip @point file | bool flip    |           | babel exec subtree | SHFT | macro-end   | open all  | selv- | debug-on     |
|--------------+-----------+--------------+--------------+------+------------------+--------------+-----------+--------------------+------+-------------+-----------+-------+--------------|
| *babel sess* | jupyClips | recenter top |              | ALT  | clip currnt file | table transp |           |                    | ALT  |             | yafolding |       | tog py break |
| *ansi*?      | access    | recenter mid |              | SUPR | clip dired  file |              |           | babel session buff | SUPR |             |           |       |              |
|--------------+-----------+--------------+--------------+------+------------------+--------------+-----------+--------------------+------+-------------+-----------+-------+--------------|
