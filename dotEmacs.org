...
|----------+-----------+--------------+--------------+------+------------------+--------------+-----------+--------------------+------+-------------+-----------+-------+--------------|
| *bash*   | mySystems | dotEmacs     | dired repos/ | CTRL | open @point file | tog truncate |           | babel exec & next  | CTRL | macro-go    | tog all   | selv+ | debug-off    |
...
|----------+-----------+--------------+--------------+------+------------------+--------------+-----------+--------------------+------+-------------+-----------+-------+--------------|
|          | jupyClips | recenter top |              | ALT  | clip currnt file | table transp |           | babel latex?       | ALT  |             | yafolding |       | tog py break |
|          | access    | recenter mid |              | SUPR | clip dired  file |              |           | babel session      | SUPR |             |           |       |              |
|----------+-----------+--------------+--------------+------+------------------+--------------+-----------+--------------------+------+-------------+-----------+-------+--------------|
...
(global-set-key [f6]       'toggle-window-split)
(global-set-key [C-f6]     'toggle-truncate-lines)
(global-set-key [S-f6]     'bool-flip-do-flip)
(global-set-key [M-f6]     'org-transpose-table-at-point)
