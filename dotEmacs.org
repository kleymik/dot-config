...
(defun my-ibuffer-hooks ()
   ...
   (define-key ibuffer-mode-map (kbd "C-TAB") 'ibuffer-toggle-filter-group)
   (message "in ibuffer"))
...
(progn
 ...
 (define-key org-mode-map [kp-5]           'outline-up-heading)
 ...
 (define-key org-mode-map [kp-add]         '(lambda () (interactive)  (org-show-children) (next-line)))  ; tbd show contents of leaf
...
;; TBD tree-pad keys for dired
;; dired treepad navigation
;; (define-key dired-mode-map [kp-4]          '(lambda () (interactive)  (outline-up-heading 2)))
(define-key dired-mode-map [kp-5]          'dired-subtree-remove)    ;; tbd if not a subdir (ie.e in rootdir), then jump  'dired-up-directory-drop-prev-dired-buffer

;; (define-key dired-mode-map [kp-multiply]   '(lambda () (interactive)  (org-backward-heading-same-level 6)))
(define-key dired-mode-map [kp-9]             'dired-subtree-previous-sibling)
;; (define-key dired-mode-map [kp-6]          'dired-subtree-insert)  ; tbd toggle with 'dired-subtree-remove ;; toggle open children
(define-key dired-mode-map [kp-3]             'dired-subtree-next-sibling)
;; (define-key dired-mode-map [kp-decimal]    '(lambda () (interactive)  (org-forward-heading-same-level 6)))              ;; moveto nth sibling, and open

(define-key dired-mode-map [kp-add]           'dired-subtree-insert)                 ;; open and moveto first child, 
...
* hooks
mostly for python -made named function so remove-hook can be used
#+BEGIN_SRC emacs-lisp
(defun py-clean-file()
...
|--------------+-----------+--------------+--------------+------+------------------+--------------+-----------+-----------------+------+-------------+-----------+-------+--------------|
| *eshell*     | doAndDone | ibuffers     | dired ~/     |      | app-open    file | transpose    | print var | tog-window-splt |      | macro-start | tog node  | selv0 | edebug fun   |
|--------------+-----------+--------------+--------------+------+------------------+--------------+-----------+-----------------+------+-------------+-----------+-------+--------------|
| *bash*       | mySystems | dotEmacs     | dired repos/ | CTRL | open @point file | bool-flip    |           |                 | CTRL | macro-go    | tog all   | selv+ | debug-off    |
| *python*     | scratch   | dotBashrc    | dired hist/  | SHFT | clip @point file | toggle-trunc |           |                 | SHFT | macro-end   | open all  | selv- | debug-on     |
|--------------+-----------+--------------+--------------+------+------------------+--------------+-----------+-----------------+------+-------------+-----------+-------+--------------|
| *babel sess* | jupyClips | recenter top |              | ALT  | clip currnt file |              |           |                 | ALT  |             | yafolding |       | tog py break |
| *ansi*?      | access    | recenter mid |              | SUPR | clip dired  file |              |           |                 | SUPR |             |           |       |              |
|--------------+-----------+--------------+--------------+------+------------------+--------------+-----------+-----------------+------+-------------+-----------+-------+--------------|
...
(global-set-key [M-f1]     '(lambda () (interactive) (org-babel-switch-to-session)))
(global-set-key [s-f1]     '(lambda () (interactive) (org-babel-execute-subtree)))
...
(global-set-key [S-f2]     '(lambda () (interactive) (find-file "/home/kleyn/history/scratch.org")))
...
(global-set-key [M-f3]     '(lambda () (interactive) (recenter-top-bottom  0)))
(global-set-key [s-f3]     '(lambda () (interactive) (recenter-top-bottom 50)))
;;(global-set-key [C-f3]   'insertLastSexp)                                                                                                                    ;; (lambda () (interactive) (dired "C:/[pm][rya][qoadmp][parsd]*" nil)))
;;(global-set-key [S-f3]   'org-save-code-block)

(global-set-key [f4]       'diredHome)
(global-set-key [C-f4]     '(lambda () (interactive) (dired "~/projects/repos" nil)))
(global-set-key [S-f4]     '(lambda () (interactive) (dired "~/history/commands/*2020*" nil)))
...
(global-set-key [S-f5]     '(lambda () (interactive)  (let ((fpth (thing-at-point 'filename t))) 
                                                                (kill-new fpth)
                                                                (message fpth))))
(global-set-key [M-f5]     'filepath-on-clipboard)
(global-set-key [s-f5]     'dired-paste-to-yank-buffer)

(global-set-key [f6]       'org-transpose-table-at-point)
(global-set-key [C-f6]     'bool-flip-do-flip)
(global-set-key [S-f6]     'toggle-truncate-lines)

(global-set-key [f7]       '(lambda () (interactive)  (let* ((currWord (thing-at-point 'symbol t))
...
(global-set-key [f12]      'edebug-defun)                        ;; put break in defun/ form whatever - REMOVE bey re-evaluating the definition
