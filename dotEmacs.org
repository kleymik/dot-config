...
(defvar syntax-table-backup (make-syntax-table (syntax-table)))
;(defvar syntax-table-backup (make-syntax-table org-mode-syntax-table))
;(defvar syntax-table-elisp-backup (make-syntax-table emacs-lisp-mode-syntax-table))
(defvar my-trsyc-syntax-table
...
(set-syntax-table my-trsyc-syntax-table)

(defun drop-angle-bracket-matching ()   ;; drop angle brackets from matching via syntax-table
...
(defun trsyc-repl ()
  "in treesycalc, the read-eval-print loop becomes:
    1) propagate values along the 'rails'
    2) evaluate the selected sexp that the cursor is in
    3) place the result after ';=' at the top of the selected sexp
    4) reformat,realign entire panel
    5) move cursor to opening paren of the selected sexp, so that repeating trsyc-repl 
      will occur one level up, on the sexp enclosing the selected sexp" 
   (interactive)
   (trsyc-propagate-rails)
   (trsyc-update-sexp-result)
   (trsyc-align-full)
   (trsyc-move cursor)
)

(defun trsyc-update-sexp-result ()
   "Eval expression and insert value on the top line of the sexp, after the ;= after the operator"
    ...
    (goto-char result-bgn)    ; go to open-paren of result
    (goto-char (line-beginning-position))                                               ; goto start of sexp =region
    (re-search-forward "\(" (line-end-position) nil)
   (backward-char 1)
) 
...
(defun trsyc-insert-panel-empty-line ()
  ...
  (save-excursion
     (insert "                         ;;;        ;;; ")                                                        (newline)
     ;(move to correct secp indent point)
  )
)
...
(defun trsyc-align ()
  ...
  (backward-up-list)
  (mark-sexp)
  (align-regexp (- (region-beginning) 0) (region-end) "\\(\\s-*\\);;;")
)

(defun trsyc-align-full ()                                                              ; mark whole panel as region and then do alignment
  "Align triple-semi-colon panes in panel"
  (interactive)
  (save-excursion
  (let ((pnl-boundary-regex  "^;-sexps-+;")                                             ; to find top or bottom of in panel, the sexp pane
        (pnl-align-regex "\\(\\s-*\\);;;"))                                             ; align-regexp for alignment pane boundaries alignment 
      (setq panel-tl (progn (re-search-backward pnl-boundary-regex)                     ; search up to get to panel top header line
                            (line-beginning-position)))
      (setq sexp-pane-bot (progn (goto-char (line-end-position))                        ; move to bottom of panel
                                  (re-search-forward-range pnl-boundary-regex nil)))    ; (message "Found rails top  %i, tl %i, tr %i" panel-tl (car rails-align-top) (cdr rails-box-bot))
      (goto-char (car sexp-pane-bot))                                               
      (align-regexp panel-tl (line-end-position) pnl-align-regex 1 0 t)                 ; realign the panel ; args: fails 1 1 t   

      (progn                                                                            ; (trsyc-clear-all-whitespace-columns)
        (goto-char (car sexp-pane-bot))                                                 ; go to bottom left
        (setq sexp-pane-bot   (re-search-forward-range pnl-boundary-regex nil)          ; redo since pane has been exanded by alignement (message "Found rails top  %i, tl %i, tr %i" panel-tl (car rails-align-top) (cdr rails-box-bot))
        (setq rectangle-as-list (extract-rectangle (car panel-tl) (cdr sexp-pane-bot))) ;  remove only-space columns inserted by alig-regexp
        (dotimes (li (length rectangle-as-list)) (message "Found rails %i %s" li (nth li rectangle-as-list)))   ; 
        (dotimes (li (length rectangle-as-list)) 
        (if  (dotimes (li (length rectangle-as-list) allSpace) (setq allSpace (and allSpace (check space))))
               (delete-region))
        (setq rectangle-as-list (trsyc-los-transpose rectangle-as-list))
        
        (count number of columns that are all-space)
        (kill-rectangle of this size))
      (progn                                                                           ; replace any spaces introducted by alignment, into top and botom panel boundary lines, with a dash "-"
        (goto-char panel-tl)
...
(defun trsyc-clear-all-whitespace-columns ()
  "use transpose to check for any columns that are completely empty, and chop them out with kill-rectangle"
  (interactive)
(save-excursion
  (let* ((pnl-boundary-regex  "^;-sexps-+;")                                                                   ; to find top or bottom of in panel, the sexp pane
         (pnl-align-regex "\\(\\s-*\\);;;")
         (panel-tl (progn (re-search-backward pnl-boundary-regex)                                                ; search up to get to panel top header line
                          (line-beginning-position)))
         (sexp-pane-bot (progn (next-line)                                                                       ; move to bottom of panel
                               (re-search-forward-range pnl-boundary-regex nil))))                               ; (message "Found rails top  %i, tl %i, tr %i" panel-tl (car rails-align-top) (cdr rails-box-bot))

      (progn                                                                                                   ; (trsyc-clear-all-whitespace-columns)
        (goto-char (car sexp-pane-bot))                                                                        ; go to bottom left
        (setq sexp-pane-bot  (re-search-forward-range pnl-boundary-regex nil))                                 ; redo since pane has been exanded by alignement (message "Found rails top  %i, tl %i, tr %i" panel-tl (car rails-align-top) (cdr rails-box-bot))
        (setq rectangle-as-list (extract-rectangle panel-tl (cdr sexp-pane-bot)))                        ; remove only-space columns inserted by alig-regexp
        (setq rectangle-as-list (trsyc-los-transpose rectangle-as-list))
        (setq numBlankCols 0)
        (dotimes (li (length rectangle-as-list) numBlankCols) 
              (setq colStr (nth li rectangle-as-list))
              (setq colStr (substring colStr 1 (- (length colStr) 1)))
              (message "Found rails %i %s XXX  %s" li colStr (eq (string-blank-p colStr) 0))  ; 0 means it has only whitespace
              (if (eq (string-blank-p colStr) 0) (setq numBlankCols (+ numBlankCols 1))))
       (setq blankColStart (- (cdr sexp-pane-bot) (car sexp-pane-bot) numBlankCols))
       (message "startCol=%i NumBlankLines=%i" blankColStart numBlankCols)

       (goto-char (+ panel-tl blankColStart)) ;(insert "X")
       (goto-char (cdr sexp-pane-bot))         ;(insert "Y")
       (kill-rectangle (+ panel-tl blankColStart) (- (cdr sexp-pane-bot) 3)) 
))))
        ;(if nil

          ;(dotimes (li (length rectangle-as-list)) (if  (dotimes (li (length rectangle-as-list) allSpace) (setq allSpace (and allSpace (check space))))  (delete-region)))
                  ;(count number of columns that are all-space)
          ;(kill-rectangle of this size))))
...
(defun trsyc-propagate-rails ()
  "process rails pane, propagating their values from source(=input) location to
  ...
  (let ((pnl-boundary-regex "^;-sexps-")                                                                       ; to find top or bottom of s-expression box in panel
    ...
    (dotimes (li (length rectangle-as-list))                                                                   ; find each rail and propagate values accordingly
       ...
       (cond ((and (string-match "i" rail)                                                                     ; check  it's a rail: has (i)nput, (o)utput and vertical dashes
            ...
            (message "selSrc XX%sXX = %s" selSrc)
            (if (string-match selSrc ";=")                                                                     ; if its a result value, strip off the leading up and including ";="
...
(global-set-key (kbd "C-x y")   'trsyc-update-sexp-result)
...
(global-set-key [M-f6]     'trsyc-update-sexp-result)
