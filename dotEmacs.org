...
|-------+----------+------------+--------------+-----------+---+----------+-----------+----+----+---+-------------+----------+------------+-----+-------|
|       | *eshell* | doAnDone   | dired ~/     | dotEmacs  |   | app-open | transpose |    |    |   | macro-start | origami  | debug-on   |     |       |
| CTRL  | *bash*   | bookmarks  | dired-omit   | transpose |   | fapp     | bool-flip |    |    |   | macro-go    | tog node | degbug-off |     | CTRL  |
| SHIFT | *python* | scratch    | dired repos/ |           |   |          |           |    |    |   | macro-end   | tog all  |            |     | SHIFT |
| ALT   | *ielm*   | mySystems? | dired hist/  |           |   |          |           |    |    |   |             | open all |            |     | ALT   |
| SUPER |          | access?    |              |           |   |          |           |    |    |   |             |          |            |     | SUPER |
|-------+----------+------------+--------------+-----------+---+----------+-----------+----+----+---+-------------+----------+------------+-----+-------|
#+BEGIN_SRC emacs-lisp
...
(global-set-key [f10]           'origami-mode)
(define-key origami-mode-map (kbd "C-<tab>")  'origami-toggle-node)
(global-set-key [C-f10]         'origami-toggle-node)
(global-set-key [S-f10]         'origami-toggle-all-nodes)
(global-set-key [M-f10]         'origami-open-all-nodes) ;  hs-hide-block

; (global-set-key [M-f2]        'buffer-menu-sort-by-filename)
(global-set-key [M-f10]         'see-shell-output)      ;; ergoemacs-open-in-external-app

(global-set-key [f11]           'set-debug-on)
(global-set-key [C-f11]         'set-debug-off)
