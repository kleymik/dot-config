...
(defun align-lisp ()
   ...
   (indent-pp-sexp)
   (mark-sexp)
   (align-regexp (region-beginning) (region-end) "\\(\\s-*\\); ")   ; align results
)
...
(defun trsyc-propagate-rails ()
  "cut rails rectangle, propagate values"
  (interactive)
  (let ((pnl-tl-regex "^;-sexps-")
        (rails-regex  ";;;-rails-+;")
        (outVal-regex "\\()\\|;;;\\)"))
    (message "-------------------------------------------------------------------")
    (search-backward-regexp "^;-sexps-")                                                                  ; search up to get to panel top header line
    ...
    (dotimes (li (length rectangle-as-list))                                                              ; find each rail and propagate values accordingly
       ...
       (cond ((and (string-match "i" rail)                                                                ; check  it's a rail: has (i)nput, (o)utput and vertical dashes
            ...
            (setq selSrc (buffer-substring-no-properties (line-beginning-position) (- (re-search-forward "\\()\\|;;;\\)") 1)))
            (setq inVal (format "%s" (my-eval-string selSrc)))
            (message "selSrc XX%sXX = %s" selSrc inVal)
            (dolist (p outValPosLst)                                                                      ; mark the sinks
                (goto-char header-line-bgn) 
                (next-line p)
                (setq selSnk (re-search-forward-range "\\()\\|;;;\\)" (line-end-position)))               ; find value to replace
                (delete-char 1)
                (insert inVal)))))            
    (message "-------------------------------------------------------------------")         
   )            
...
(defun re-search-forward-range (regex bound)
   ...
   (cons  
      (re-search-forward regex bound nil)   ; (forward-sexp) doesn't work
      (match-beginning 0))
)
